...
public class RWayTrieST<V> implements StringST<V> {
	...
	private String floor(Node node, String key, int digit, String prefix, String lastKeyFound, boolean mustBeEqualDigit) {
		if (node == null)
			return null;
		...
		for (char nextChar = currentChar; true; nextChar--) {
			if (node.next[nextChar] != null) {
				...
				lastKeyFound = floor(node.next[nextChar], key, digit + 1, prefix + nextChar, lastKeyFound,
						mustBeEqualDigit);
	...
	public static void main(String[] args) {
		RWayTrieST<Integer> arvore = new RWayTrieST<>();
...
//		arvore.put("moeda", "moeda");
		...
		//for (String k : arvore.keysThatMatch(pat))
