...
public class TST<V> implements StringST<V> {
	...
	public Iterable<String> keysThatMatch(String str) {
		...
		collect(root, " ", str, keysThatMatch);
		return keysThatMatch;
	}

	private void collect(Node node, String prefix, String pattern, LinkedList<String> lista) {
		if (node == null)
			return;
		...
		if (nextCharInPattern == '.' || nextCharInPattern == node.ch) {
			if (digit == pattern.length() - 1 && node.value != null) {
				...
				collect(node.mid, prefix + node.ch, pattern, lista);
			}
		}
	}
    ...
    public String floor(String key) {
        ...
        return floor(root, key, 0, " ", null, true);
    }

    private String floor(Node node, String key, int digit, String prefix, String lastKeyFound,
                         boolean mustBeEqualDigit) {
        if (node == null) return lastKeyFound;

        String prefixWithCharacter = prefix + node.ch;
        ...
        if (currentChar < node.ch && mustBeEqualDigit) {
            ...
            if (prefixWithCharacter.compareTo(key) > 0) {
