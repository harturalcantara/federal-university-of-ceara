...
public class TST<V> implements StringMap<V> {
	...
	private int search(Node node, String query, int digit, int length) {
		if (node == null)
			return length;
		if (node.value != null)
			length = digit + 1; //obs: estranho esse +1 
	...
	public static void main(String[] args) {
		...
		System.out.println("Delete - Deletenado keys");
		arvore.delete("Rene");
		arvore.delete("Tree");
		arvore.delete("Trie");		
		for (String str : arvore.keys())
		...
		System.out.println("Rank: TST");
		System.out.println(arvore.rank("TST"));
